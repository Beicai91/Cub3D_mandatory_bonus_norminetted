...
void	create_screen_img(t_data *data, double wall_hgt, int x)
{
	...
	top_y = SCREEN_HGT / 2 - wall_hgt / 2 + data->player.dir_y;
	bottom_y = SCREEN_HGT / 2 + wall_hgt / 2 + data->player.dir_y;
...
static void	raycasting2(t_data *data)
{
	if (data->rayinfo.ray_hd < data->rayinfo.ray_vd)
	{
		...
		data->rayinfo.hit_door = data->rayinfo.h_hit_door;
	}
	else
	{
		...
		data->rayinfo.hit_door = data->rayinfo.v_hit_door;
	}
