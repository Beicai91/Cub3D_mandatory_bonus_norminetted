...
/*   Updated: 2024/08/21 12:12:54 by bcai             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_texture_maps2(t_data *d)
{
	...
	d->tex.ea_img.addr = (int *)mlx_get_data_addr(d->tex.ea_img.img_ptr,
			...
			&(d->tex.ea_img.endian));
	d->tex.door_img.img_ptr = mlx_xpm_file_to_image(d->mlx, d->dotex,
			&(d->tex.door_img.width), &(d->tex.door_img.height));
	if (!d->tex.door_img.img_ptr)
		error_exit("Error:\n failed to open texture image(door)\n", d);
	d->tex.door_img.addr = (int *)mlx_get_data_addr(d->tex.door_img.img_ptr,
			&(d->tex.door_img.bpp), &(d->tex.door_img.line_sz),
			&d->tex.ea_img.endian);
}
