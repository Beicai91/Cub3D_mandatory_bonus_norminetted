...
/*   Updated: 2024/08/21 18:22:56 by bcai             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_first_line(t_data *data)
{
	...
	while (map[0][j])
	{
		...
		if (map[0][j] == ' ' || map[0][j] == '\t')
		{
			if ((j - 1 >= 0 && ft_strchr("0NSWE", map[0][j - 1])) || (j
					+ 1 <= data->mapinfo.max_w - 1 && ft_strchr("0NSWE",
						map[0][j + 1])) || ft_strchr("0NSWE", map[1][j]))
				error_exit("Map is not closed: check first line\n", data);
		}
		j++;
	}
...
void	check_last_line(t_data *data)
{
	...
	while (map[i][j])
	{
		...
		if (map[i][j] == ' ' || map[i][j] == '\t')
		{
			if ((j - 1 >= 0 && ft_strchr("0NSWE", map[i][j - 1])) || (j
					+ 1 <= data->mapinfo.max_w - 1 && ft_strchr("0NSWE",
						map[i][j + 1])) || ft_strchr("0NSWE", map[i - 1][j]))
				error_exit("Map is not closed: check last line\n", data);
		}
		j++;
	}
...
{
	if ((j - 1 >= 0 && ft_strchr("0NSWE", map[i][j - 1])) || (j
			+ 1 <= data->mapinfo.max_w - 1 && ft_strchr("0NSWE", map[i][j
				+ 1])) || ft_strchr("0NSWE", map[i - 1][j])
		|| ft_strchr("0NSWE", map[i + 1][j]))
...
void	check_wall(t_data *data)
{
	...
	{
		if (ft_strchr("0NSWE", map[i][0]) || ft_strchr("0NSWE",
